...
- [v] Create `frontend/src/services/CatchService.ts`:
...
- [v] Create `frontend/src/services/AnalyticsService.ts`:
...
- [v] Update `frontend/src/pages/Dashboard.tsx`:
...
- [v] Verify that each catch is correctly associated with a session (though Dashboard is read-only, this logic is critical for Plan 005).
- [v] Ensure fish names are correctly resolved from the Fish API.
