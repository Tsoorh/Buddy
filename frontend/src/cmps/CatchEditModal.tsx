...
const CatchEditModal: React.FC<CatchEditModalProps> = ({
  catchItem,
  isOpen,
  onClose,
  onSuccess,
  ...
  const getErrors = () => {
    ...
    if (!formData.fish_id) errors.fish_id = "Species is required";
    ...
    if (catchItem) {
      setFormData({
        ...
        catch_time: catchItem.catch_time
          ? new Date(catchItem.catch_time).toISOString().slice(0, 16)
          : "",
      });
    }
  }, [catchItem]);
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Edit Catch">
      <form onSubmit={onHandleSubmit} className="auth-form">
        {error && (
          <div className="alert alert-danger border-0 glass-card text-white mb-3 small">
            {error}
          </div>
        )}
        ...
        <div className="mb-3">
          ...
          <select
            ...
            onChange={(e) =>
              setFormData({ ...formData, fish_id: e.target.value })
            }
          >
        ...
        <button
          type="submit"
          className="btn btn-accent w-100 py-3 mt-2"
